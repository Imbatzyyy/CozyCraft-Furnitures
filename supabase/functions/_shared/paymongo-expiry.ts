import { findPaidProviderPayment, providerSessionLivemode } from "./paymongo-session.ts";
...
const providerError = (payload: unknown, fallback: string) => {
  ...
  const errors = (payload as Record<string, any>).errors;
...
const settlePaidSession = async ({
  ...
  if (error) {
    return {
      outcome: "retry",
      message: error.message ?? "The completed payment could not be reconciled yet.",
    };
  }
...
const finalizeExpiredSession = async ({
  ...
  if (snapshotError) {
    return {
      outcome: "retry",
      message: snapshotError.message ?? "The expired payment session could not be recorded.",
    };
  }
  ...
  if (data === false) {
    return {
      outcome: "retry",
      message: "The order changed while its payment session was expiring and will be checked again.",
    };
  }
...
export const reconcileElapsedPaymongoSession = async ({
  ...
  try {
    const initial = await fetchSession(transaction.provider_session_id, secretKey);
    if (!initial.response.ok) {
      ...
      return {
        outcome: "retry",
        message: providerError(initial.payload, "PayMongo could not verify the payment session."),
      };
    }
    ...
    const expireResponse = await fetch(`${sessionUrl(transaction.provider_session_id)}/expire`, {
      method: "POST",
      headers: authHeaders(secretKey),
      signal: AbortSignal.timeout(12_000),
    });
    ...
    const verified = await fetchSession(transaction.provider_session_id, secretKey);
