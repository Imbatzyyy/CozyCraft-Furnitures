...
export const findPaidProviderPayment = (payload: unknown): PaidProviderPayment | null => {
  ...
  for (const candidate of payments) {
    ...
    const paymentAttributes = record(payment?.attributes) ?? record(nested?.attributes);
