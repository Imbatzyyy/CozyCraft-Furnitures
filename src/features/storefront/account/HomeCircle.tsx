...
export function HomeCircleView({ snapshot, busy, error, refresh, userId = "" }: { snapshot: CircleSnapshot | null; busy: boolean; error: string; refresh: () => void; userId?: string }) {
  ...
  const wallet = useMemo(() => snapshot ? {rows:snapshot.rewards,hasMore:Boolean(snapshot.rewardHasMore)} : undefined,[snapshot]);
  ...
  return <section className="web-circle" aria-labelledby="circle-title">
    ...
    <VoucherExchange userId={userId} points={Number(account?.points_balance || 0)} disabled={!account || busy || Boolean(error)} onClaimed={refresh}/>
    <section className="circle-section"><p className="account-eyebrow">YOURS TO ENJOY</p><h3>Available rewards</h3>{wallet ? <VoucherWallet key={snapshot?.account.points_balance + ':' + snapshot?.rewards.map(r=>r.id).join(',')} userId={userId} initial={wallet}/> : <p className="circle-empty">Your vouchers will appear once your membership loads.</p>}<p className="circle-small">Six vouchers per page. Choose a voucher here or select one directly at checkout. Expired and used vouchers cannot be applied.</p></section>
