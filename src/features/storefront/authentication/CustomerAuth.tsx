...
} from "@/lib/auth/customer-auth-destination";
...
export function Account({ mode }: { mode: "login" | "signup" }) {
  ...
  const checkoutIntent = isCheckoutAuthDestination(customerDestination);
  const alternateAuthHref = customerAuthHref(
        ...
        <section className="flex min-h-0 justify-center overflow-y-auto px-5 py-7 sm:px-9 sm:py-9 lg:px-12 xl:px-10 xl:py-8 2xl:px-16">
          <form onSubmit={submit} className="auth-fixed-form my-auto w-full max-w-xl">
            ...
            <p className="mt-2 text-sm leading-5 text-muted-foreground">
              {checkoutIntent
                ? mode === "login"
                  ? "Sign in to continue checkout. Your bag is safely saved, and we’ll return you to checkout when you’re done."
                  : "Create an account to continue checkout. Your bag will stay saved while you confirm your email."
                : mode === "login"
...
export function ResetPassword() {
  ...
  const [error, setError] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const passwordMinimum = Math.max(
    8,
    storeSettings.account_settings.password_minimum_length,
  );
  ...
  const savePassword = async (event: FormEvent) => {
    ...
    if (!meetsPasswordPolicy) {
      setError(
        `Use at least ${passwordMinimum} characters with uppercase, lowercase, a number, and a symbol.`,
      );
      return;
    }
    setSubmitting(true);
        ...
        <section className="auth-fixed-form w-full max-w-md">
          ...
          {status === "saved" && (
            <div className="mt-10 rounded-3xl bg-[#eee8df] p-7">
              ...
              <p className="mt-4 text-sm leading-6 text-muted-foreground">
                Your CozyCraft password has been saved. You can now sign in
                with your email and password or continue using Google.
              </p>
              <button
                onClick={() => nav("/login")}
          ...
          {status === "ready" && (
            <form onSubmit={savePassword} className="mt-10">
              ...
              <h1 className="mt-4 font-serif text-4xl sm:text-5xl">
                {setupMode ? "Create your CozyCraft password." : "Choose something new."}
              </h1>
              <p className="mt-4 text-sm leading-6 text-muted-foreground">
