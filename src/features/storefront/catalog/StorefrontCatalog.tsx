...
export function Home() {
  ...
  const editProducts = products.slice(0, 8);
  ...
  useEffect(() => {
    ...
    nextImage.src = slides[(active + 1) % slides.length].image;
  }, [active, slides]);
      ...
      <main>
        ...
        <section id="shop" className="border-y border-border bg-card">
          <div className="mx-auto max-w-[1440px] px-5 py-16 lg:px-10 lg:py-24">
            ...
            <div className="mt-12 flex justify-center">
              <Link
                to="/new-arrivals"
                className="inline-flex min-h-11 items-center gap-2 rounded-full border border-border bg-background px-5 text-xs font-semibold transition hover:bg-secondary"
              >
        ...
        <section className="mx-auto max-w-[1440px] px-5 py-20 lg:px-10 lg:py-28">
          <div className="flex flex-col items-start justify-between gap-7 border-y border-foreground py-10 sm:flex-row sm:items-end">
            ...
            <form className="w-full max-w-sm" onSubmit={joinNewsletter} noValidate>
              <div
              ...
              >
                <input
                  ...
                  className="h-12 min-w-0 flex-1 bg-transparent pr-4 text-sm outline-none placeholder:text-muted-foreground"
                  placeholder="Your email address"
                />
                <button
                  type="submit"
                  disabled={newsletterState === "submitting"}
                  className="min-w-[4.5rem] text-right text-sm font-semibold transition-opacity disabled:cursor-wait disabled:opacity-55"
                >
...
export function CollectionPage() {
  ...
  if (query.trim()) {
    items = rankCatalogSearch(items, expandedQuery);
  }
  ...
  if (!searchActive || sort !== "featured") items = sortProducts(items, sort);
    ...
    <Layout>
      <main>
        <section className="mx-auto max-w-[1440px] px-5 pt-5 lg:px-10">
          <div className="relative min-h-[300px] overflow-hidden rounded-3xl bg-secondary sm:min-h-[420px]">
            <ResilientImage
              src={info.image}
              alt={info.title}
              loading="eager"
              className={`absolute inset-0 h-full w-full object-cover ${info.title === "Dining room" ? "object-[center_72%]" : "object-center"}`}
            />
            <div className="absolute inset-0 bg-black/42" />
            <div className="relative mx-auto flex min-h-[300px] max-w-[1440px] flex-col justify-end px-6 py-8 text-white sm:min-h-[420px] sm:px-12 sm:py-12">
        ...
        <section className="mx-auto max-w-[1440px] px-5 py-8 sm:py-12 lg:px-10 lg:py-16">
        ...
        {compareIds.length > 0 && (
          <div className="fixed inset-x-3 bottom-[calc(var(--mobile-store-nav-height)+1rem)] z-40 mx-auto flex max-w-xl items-center gap-3 rounded-2xl bg-[#201f1d] px-4 py-3 text-white shadow-2xl md:bottom-5">
              ...
              <div className="flex items-center gap-2">
                {compared.length > 1 && (
                  <button
                    ...
                    className={`inline-flex min-h-9 items-center gap-2 rounded-full border px-3 text-[10px] font-semibold transition ${showDifferencesOnly ? "border-foreground bg-foreground text-background" : "border-border bg-card hover:bg-secondary"}`}
                  >
                    <SlidersHorizontal size={13} />
                    Differences only
                  </button>
                )}
                <p className="text-[10px] text-muted-foreground sm:hidden">Swipe →</p>
              </div>
            </div>
        ...
        <main className="mx-auto grid min-h-[62vh] max-w-[1440px] place-items-center px-5 py-12 lg:px-10">
          {availability !== "unavailable" ? (
            ...
            <section className="w-full max-w-xl rounded-[2rem] border border-border bg-card p-7 text-center shadow-[0_18px_50px_rgba(35,31,27,.08)] sm:p-10">
              ...
              <p className="mt-6 text-[10px] font-bold tracking-[.18em] text-muted-foreground">PRODUCT UNAVAILABLE</p>
              ...
              <Link to="/home#shop" className="mt-7 inline-flex min-h-12 items-center justify-center rounded-xl bg-foreground px-6 text-sm font-semibold text-background">
    ...
    <Layout>
      <main className="mx-auto max-w-[1440px] px-5 py-7 lg:px-10 lg:py-10">
        ...
        <div className="grid gap-9 lg:grid-cols-[1.1fr_.9fr] lg:gap-14">
          <section>
            <button
              ...
              className="group relative block aspect-[.93] w-full overflow-hidden bg-secondary text-left"
              aria-label={`Open fullscreen image ${photo + 1} of ${product.images.length} for ${product.name}`}
            >
              <ResilientImage
              ...
              />
              <span className="absolute right-3 top-3 inline-flex min-h-10 items-center gap-2 rounded-full bg-black/65 px-3 text-[10px] font-semibold text-white shadow-lg backdrop-blur-sm">
                <Eye size={14} /> View full screen
              </span>
              <span className="absolute bottom-3 right-3 rounded-full bg-black/65 px-2.5 py-1 text-[10px] font-semibold text-white backdrop-blur-sm">
                {photo + 1} / {product.images.length}
              </span>
            </button>
            <div className="mt-3 grid grid-cols-3 gap-3">
              {product.images.map((image, i) => (
                <button
                  ...
                  aria-label={`Show ${product.name} image ${i + 1} of ${product.images.length}`}
            ...
            <p className="mt-4 text-center text-[11px] text-muted-foreground">
              Select a view, then tap the main image to zoom and swipe.
            </p>
          </section>
          <section className="lg:pt-4">
            ...
            <div className="mt-8 border-y border-border py-5 text-sm">
              ...
              <div className="flex justify-between py-2">
                ...
                <span
                  className={!liveAvailable || outOfStock ? "text-[#9a493f]" : lowStock ? "text-[#8b6b36]" : "text-[#62755a]"}
                  aria-label={`Availability: ${liveAvailable ? stockAvailability : "Currently unavailable"}`}
                >
                  ● {liveAvailable ? stockAvailability : "Currently unavailable"}
                </span>
              </div>
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button type="button" disabled={!liveAvailable} onClick={() => { const result = toggleComparedProduct(product.id); setCompared(result.ids.includes(product.id)); setAlertNotice(result.limitReached ? "Compare up to four products. Remove one first." : result.added ? "Added to comparison." : "Removed from comparison."); }} className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-45 ${compared ? "border-foreground bg-foreground text-background" : "border-border bg-card"}`}><Scale size={14}/>{compared ? "In comparison" : "Compare"}</button>
              {outOfStock && <button type="button" disabled={!liveAvailable || alertBusy === "back_in_stock"} onClick={() => void toggleAlert("back_in_stock")} className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-45 ${alerts.includes("back_in_stock") ? "border-[#6d8065] bg-[#e7eee3] text-[#50664b]" : "border-border bg-card"}`}><Bell size={14}/>{alerts.includes("back_in_stock") ? "Back-in-stock alert on" : "Notify when available"}</button>}
              <button type="button" disabled={!liveAvailable || alertBusy === "price_drop"} onClick={() => void toggleAlert("price_drop")} className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-45 ${alerts.includes("price_drop") ? "border-[#6d8065] bg-[#e7eee3] text-[#50664b]" : "border-border bg-card"}`}><Bell size={14}/>{alerts.includes("price_drop") ? "Price alert on" : "Alert me at 10% off"}</button>
            </div>
            ...
            <div className="mt-7 flex gap-3">
              ...
              >
                <ShoppingBag size={17} />
                {!liveAvailable ? "Unavailable" : outOfStock ? "Out of stock" : "Add to bag"}
              </button>
              <button
                onClick={() => toggle(product.id)}
                disabled={!liveAvailable}
                className="grid h-12 w-12 place-items-center border border-border disabled:cursor-not-allowed disabled:opacity-40"
        ...
        {productGalleryOpen && createPortal(
          <div
            className="fixed inset-0 z-[350] grid place-items-center bg-black/90 p-2 backdrop-blur-sm sm:p-6"
            role="dialog"
            aria-modal="true"
          ...
          >
            <section ref={productGalleryPanelRef} className="flex h-[min(94dvh,900px)] w-full max-w-6xl flex-col overflow-hidden rounded-[1.5rem] border border-white/10 bg-[#171614] text-white shadow-2xl">
              <header className="flex min-h-16 items-center justify-between gap-3 border-b border-white/10 px-4 py-3 sm:px-5">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-[10px] text-white/60">Image {photo + 1} of {product.images.length} · Swipe or use arrow keys</p>
                </div>
              ...
              >
                ...
                {product.images.length > 1 && (
                  <>
                    ...
                    <button type="button" onClick={() => setPhoto((current) => (current + 1) % product.images.length)} className="absolute right-3 grid h-11 w-11 place-items-center rounded-full bg-black/70 shadow-lg transition hover:bg-black sm:right-5" aria-label="Next product image"><ChevronRight size={22}/></button>
                  </>
                )}
              </div>
              <footer className="flex gap-2 overflow-x-auto border-t border-white/10 p-3 sm:justify-center sm:p-4">
                {product.images.map((image, index) => (
                  <button key={`${image}-fullscreen`} type="button" onClick={() => setPhoto(index)} aria-pressed={photo === index} aria-label={`Show image ${index + 1}`} className={`h-14 w-14 shrink-0 overflow-hidden rounded-lg border-2 sm:h-16 sm:w-16 ${photo === index ? "border-white" : "border-transparent opacity-60 hover:opacity-100"}`}>
        ...
        <div className="fixed inset-x-0 bottom-[var(--mobile-store-nav-height)] z-30 flex items-center gap-3 border-t border-border bg-card/97 px-4 py-3 shadow-[0_-10px_30px_rgba(35,31,27,.12)] backdrop-blur md:hidden">
          ...
          <button onClick={() => toggle(product.id)} disabled={!liveAvailable} aria-label={isSaved ? "Remove from wishlist" : "Add to wishlist"} className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-border disabled:cursor-not-allowed disabled:opacity-40"><Heart size={17} fill={isSaved ? "currentColor" : "none"} /></button>
          <button onClick={() => add(product.id, quantity)} disabled={!liveAvailable || outOfStock} className="h-11 rounded-xl bg-foreground px-5 text-xs font-semibold text-background disabled:cursor-not-allowed disabled:opacity-50">{!liveAvailable ? "Unavailable" : outOfStock ? "Out of stock" : "Add to bag"}</button>
        </div>
      </main>
      {availabilityDialogOpen && createPortal(
        <div className="fixed inset-0 z-[400] grid place-items-center bg-[#1f1d1a]/65 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="product-unavailable-title">
          <section className="relative w-full max-w-md overflow-hidden rounded-[1.75rem] border border-white/15 bg-[#fbfaf7] p-7 shadow-2xl sm:p-9">
            ...
            <p className="mt-6 text-[10px] font-bold tracking-[.18em] text-muted-foreground">AVAILABILITY UPDATE</p>
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              <button type="button" onClick={() => setAvailabilityDialogOpen(false)} className="min-h-12 rounded-xl border border-border px-4 text-sm font-semibold">Stay on page</button>
              <button type="button" onClick={() => nav(collectionPath)} className="min-h-12 rounded-xl bg-foreground px-4 text-sm font-semibold text-background">View similar pieces</button>
            </div>
          </section>
        </div>,
        document.body,
